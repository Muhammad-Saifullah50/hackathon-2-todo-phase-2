...
- [ ] T001 Create monorepo directory structure (backend/, frontend/, docker-compose.yml at root)
- [ ] T002 [P] Initialize backend Python project with pyproject.toml
- [ ] T003 [P] Initialize frontend Next.js project with package.json
- [ ] T004 [P] Create root-level .gitignore (exclude .env, node_modules, __pycache__, .venv, .next)
- [ ] T005 [P] Create backend/.gitignore (exclude .env, __pycache__, .pytest_cache, .venv)
- [ ] T006 [P] Create frontend/.gitignore (exclude .env.local, .next, node_modules)
...
- [ ] T007 [P] Create backend/requirements.txt with FastAPI, SQLModel, Alembic, asyncpg, uvicorn, pydantic-settings
- [ ] T008 [P] Create backend/pyproject.toml with ruff, black, mypy, pytest configuration
- [ ] T009 [P] Create backend/.env.example with DATABASE_URL, SECRET_KEY, CORS_ORIGINS, LOG_LEVEL
- [ ] T010 Create backend/src/__init__.py (empty init file)
- [ ] T011 Create backend/src/config.py with Pydantic Settings class for environment variables
- [ ] T012 Create backend/src/database.py with async SQLModel engine, session factory, and get_session dependency

### Backend Database Models

- [ ] T013 [P] Create backend/src/models/__init__.py
- [ ] T014 [P] Create backend/src/models/user.py with User, UserBase, UserCreate, UserResponse models
- [ ] T015 [P] Create backend/src/models/task.py with Task, TaskBase, TaskCreate, TaskUpdate, TaskResponse models

### Backend API Schemas

- [ ] T016 [P] Create backend/src/schemas/__init__.py
- [ ] T017 [P] Create backend/src/schemas/responses.py with StandardizedResponse, ErrorResponse, PaginationMeta models

### Backend Infrastructure

- [ ] T018 [P] Create backend/src/services/__init__.py (empty for now)
- [ ] T019 [P] Create backend/src/validators/__init__.py (empty for now)
- [ ] T020 [P] Create backend/src/api/__init__.py
...
- [ ] T028 [P] Create frontend/components/providers.tsx with TanStack Query provider
- [ ] T029 [P] Create frontend/lib/utils.ts with cn() utility function
- [ ] T030 [P] Create frontend/lib/api.ts with Axios client and request/response interceptors
- [ ] T031 [P] Create frontend/lib/validations.ts (empty for now)

### Initialize Shadcn/ui

- [ ] T032 Run npx shadcn@latest init in frontend/ directory to initialize Shadcn/ui
- [ ] T033 [P] Install Shadcn/ui button component in frontend/components/ui/button.tsx
- [ ] T034 [P] Install Shadcn/ui card component in frontend/components/ui/card.tsx
- [ ] T035 [P] Install Shadcn/ui toast component in frontend/components/ui/toast.tsx
...
- [ ] T040 [P] [US1] Create backend/tests/__init__.py
- [ ] T041 [P] [US1] Create backend/tests/conftest.py with test database fixtures and test client
- [ ] T042 [P] [US1] Create backend/tests/unit/__init__.py
- [ ] T043 [P] [US1] Create backend/tests/integration/__init__.py
- [ ] T044 [US1] Create backend/tests/integration/test_health.py with smoke test for GET /health endpoint
...
- [ ] T047 [US1] Create backend/src/main.py with FastAPI app, CORS middleware, and route registration
- [ ] T048 [US1] Create backend/src/api/health.py with GET /health endpoint (returns service status and database connectivity)
...
- [ ] T117 [P] [US4] Add pytest, pytest-asyncio, pytest-cov, httpx to backend/requirements.txt
- [ ] T118 [P] [US4] Configure pytest in backend/pyproject.toml (testpaths, asyncio_mode, coverage)
- [ ] T119 [US4] Update backend/tests/conftest.py with comprehensive fixtures (async_client, test_db_session, mock_user, mock_task)
...
- [ ] T140 [US4] Verify all smoke tests are independent and can run in isolation
